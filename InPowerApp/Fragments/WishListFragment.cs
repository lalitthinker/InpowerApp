...
{
    public class WishListFragment : Android.Support.V4.App.Fragment
    {
        bool loadList = true;
        PaginationModel paginationModel = new PaginationModel();
        RecyclerView mRecyclerView;
        RecyclerView.LayoutManager mLayoutManager;
        WishListBookListAdapter mAdapter;
        ...
        public void loadBookAdapter()
        {
            try
            {
                ...
                {
                    mAdapter = new WishListBookListAdapter(BookList, this.Context);
        ...
        public void RealodloadBookAdapter(int book)
        {
            try
            {
                Books bookitem = BookList.Where(a => a.BookId == book).FirstOrDefault();
        ...
        private void ListReload(object sender, int e)
        {
            if (e != 0)
                RealodloadBookAdapter(e);
        }

        public override void OnCreateOptionsMenu(IMenu menu, MenuInflater inflater)
        {
            inflater.Inflate(Resource.Menu.action_wishList, menu);
            base.OnCreateOptionsMenu(menu, inflater);

            if (BookList.Count > 0)
            {

                var searchItems = menu.FindItem(Resource.Id.action_WishListBookSearch);
